#include "CeilVisual.h"
#include <cmath>

CeilVisualArea::CeilVisualArea()
{
...
void CeilVisualArea::setup(double n, int o, QColor inColor, QColor outColor)
{
    clear();
    prepareSizeVariables(o);

    double totalDelay = std::max(0.0,500*std::log2(o - 1));
    ...
    if(!qFuzzyIsNull(n - o))
    {
        QQmlComponent comp(qmlEngine(this), QUrl(QStringLiteral("qrc:/QML/TileRectangles/CeilFinalRectangle.qml")));

        QVariantMap map;
        map["width"] = (1.0 - (o - n)) * (tSize());
        map["initialWidth"] = (1.0 - (o - n)) * (tSize());
        map["height"] = tSize();
        map["x"] = 0.5 + m_spacing * ((o - 1) % m_xBoxNum);
        map["y"] = 0.5 + m_spacing * std::floor((o - 1)/m_xBoxNum);
        map["iColor"] = inColor;
        map["fColor"] = outColor;
...
void CeilVisualArea::prepareSizeVariables(int totalBoxes)
{
    if(totalBoxes <= 10)
    {
        m_borderWidth = 8;
