...
void Wire::mouseMoveEvent(QMouseEvent *event)
{
    ...
    QPointF scaledDisplacement = m_initialDisplacement + (m_currentScreenPosition - m_initialScreenPosition)/ corkboard()->scale();
...
void Wire::showConnectionCursor(bool validConnection)
{
    ...
    {
        QCursor cursor = QCursor(QPixmap(":/Images/ConnectWire.png"),2,2);
        QGuiApplication::setOverrideCursor(cursor);
    }
    else
    {
        QCursor cursor = QCursor(QPixmap(":/Images/StylizedCursor2.png"),2,2);
        QGuiApplication::setOverrideCursor(cursor);
    }
}
